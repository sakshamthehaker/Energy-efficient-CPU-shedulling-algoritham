...
class EOTS:
    ...
    def setup_gui(self):
        ...
        # Running Process Information Section with Scrollbars
        running_frame = ttk.LabelFrame(self.root, text="Running and Past Process Information")
        running_frame.pack(padx=10, pady=5, fill="both")

        self.running_canvas = tk.Canvas(running_frame)
        self.running_text = tk.Text(self.running_canvas, height=6, width=100, state='disabled', wrap='none')  # Increased height and width
        v_scrollbar = ttk.Scrollbar(running_frame, orient="vertical", command=self.running_text.yview)
        h_scrollbar = ttk.Scrollbar(running_frame, orient="horizontal", command=self.running_text.xview)
        self.running_text.configure(yscrollcommand=v_scrollbar.set, xscrollcommand=h_scrollbar.set)

        self.running_canvas.pack(side=tk.LEFT, fill="both", expand=True)
        v_scrollbar.pack(side=tk.RIGHT, fill="y")
        h_scrollbar.pack(side=tk.BOTTOM, fill="x")
        self.running_canvas.create_window((0, 0), window=self.running_text, anchor="nw")
        self.running_text.bind("<Configure>", lambda e: self.running_canvas.configure(scrollregion=self.running_canvas.bbox("all")))
    ...
    def update_running_processes(self):
        with self.gui_lock:
            ...
            # Header for Running Processes
            running_header = f"{'RUNNING PROCESSES':^60}\n"
            self.running_text.insert(tk.END, running_header)
            ...
            # Header for Past Processes
            past_header = f"{'PAST PROCESSES':^60}\n"
            self.running_text.insert(tk.END, past_header)
            past_header = f"{'Core':<6} {'Task ID':<8} {'Priority':<9} {'Frequency':<10} {'Exec Time (ms)':<15} {'Finish Time (ms)':<20}\n"
            self.running_text.insert(tk.END, past_header)
            
            # Display Past Processes
            for core in self.cores:
